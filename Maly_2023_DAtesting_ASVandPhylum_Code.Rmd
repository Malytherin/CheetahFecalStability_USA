...
# lmc test -----------------------------------------------
S8_DA.lmc <- DA.lmc(S8s.pre, predictor = "SampleDay", paired = "Fecal.ID", allResults = TRUE)
...
# lmc post hoc
S8_ASV_DA.lmc <- DA.lmc(y1, predictor = "TYPE2", allResults = TRUE)
lmc_ASV.tukeys <- DA.lsmeans(S8_DA.lmc, variable = "predictor") 
View(lmc_ASV.tukeys) # look at pvalue adjusted
...
# llm2 test ----------------------------------------------
S8_DA.llm2 <- DA.llm2(S8s.pre, predictor = "SampleDay", paired = "Fecal.ID")
S8_DA.llm2[S8_DA.llm2$pval.adj < 0.05, "Feature"]

# llm2 post hoc
S8_DA.llm2 <- DA.llm2(S8s.pre, predictor = "SampleDay", paired = "Fecal.ID", allResults = TRUE)
...
# llm test ----------------------------------------------------
S8_DA.llm <- DA.llm(S8s.pre, predictor = "SampleDay", paired = "Fecal.ID")
S8_DA.llm[S8_DA.llm$pval.adj < 0.05, "Feature"]

# llm2 post hoc
S8_DA.llm <- DA.llm(S8s.pre, predictor = "SampleDay", paired = "Fecal.ID", allResults = TRUE)
...
View(llm2_D0v2) #ASVs: 26, 38, 48, 1, 119, 88
llm_D0v2 <- llm.ASV_tukey_D0vAll[llm.ASV_tukey_D0vAll[,7] <= 0.05,]
View(llm_D0v2) #ASVs: 26, 38, 1, 119, 154
# Day 0 vs Day 2 ASVs: 119, 38, 1, 26

lmc_D0v3 <- lmc.ASV_tukey_D0vAll[lmc.ASV_tukey_D0vAll[,8] <= 0.05,]
View(lmc_D0v3) #ASVs: 119, 34, 48, 38, 1, 46, 12, 26, 5, 13, 27, 86, 33, 88, 71, 4, 17
llm2_D0v3 <- llm2.ASV_tukey_D0vAll[llm2.ASV_tukey_D0vAll[,8] <= 0.05,]
View(llm2_D0v3) #ASVs: 17, 46, 26, 98, 71, 31, 38, 62, 4, 34, 64, 48, 12, 1, 16, 111, 22, 119, 19, 18, 88
llm_D0v3 <- llm.ASV_tukey_D0vAll[llm.ASV_tukey_D0vAll[,8] <= 0.05,]
View(llm_D0v3) #ASVs: 17, 46, 26, 38, 4, 34, 64, 48, 12, 1, 86, 119, 154
# Day 0 vs Day 3 ASVs: 119, 34, 48, 38, 4, 1, 46, 12, 26, 86, 88, 71, 17

lmc_D0v4 <- lmc.ASV_tukey_D0vAll[lmc.ASV_tukey_D0vAll[,9] <= 0.05,]
View(lmc_D0v4) #ASVs: 119, 34, 64, 48, 38, 1, 46, 12, 26, 5, 86, 4, 17
llm2_D0v4 <- llm2.ASV_tukey_D0vAll[llm2.ASV_tukey_D0vAll[,9] <= 0.05,]
View(llm2_D0v4) #ASVs: 17, 46, 26, 38, 4, 34, 64, 48, 12, 1, 119, 88
llm_D0v4 <- llm.ASV_tukey_D0vAll[llm.ASV_tukey_D0vAll[,9] <= 0.05,]
View(llm_D0v4) #ASVs: 26, 38, 34, 64, 48, 119
# Day 0 vs Day 4 ASVs: 119, 34, 64, 48, 38, 1, 46, 12, 26, 4, 17

lmc_D0v5 <- lmc.ASV_tukey_D0vAll[lmc.ASV_tukey_D0vAll[,10] <= 0.05,]
View(lmc_D0v5) #ASVs: 119, 38, 1, 46, 12, 26, 13, 4
llm2_D0v5 <- llm2.ASV_tukey_D0vAll[llm2.ASV_tukey_D0vAll[,10] <= 0.05,]
View(llm2_D0v5) #ASVs: 17, 46, 26, 38, 4, 34, 48, 12, 1, 13, 88
llm_D0v5 <- llm.ASV_tukey_D0vAll[llm.ASV_tukey_D0vAll[,10] <= 0.05,]
View(llm_D0v5)

S8_DA_tukeyASVs <- c("ASV 119", "ASV 38", "ASV 1", "ASV 26", "ASV 34", "ASV 48", "ASV 4", "ASV 46", "ASV 12", "ASV 86", "ASV 88", "ASV 71", "ASV 17", "ASV 64", "ASV 13")
# Use for Figure 2
```
...
# lmc test -----------------------------------------------------
S8_P.lmc <- DA.lmc(S8_phylum, predictor = "SampleDay", paired = "Fecal.ID", allResults = TRUE) # added the allResults = TRUE because it needed that arguement to run the tukey post hoc tests but now the following line (931) doesn't work, so I'm not sure which is better/easier to look at
View(S8_P.lmc)
...
# llm2 test -----------------------------------------------------
S8_P.llm2 <- DA.llm2(S8_phylum, predictor = "SampleDay", paired = "Fecal.ID", allResults = TRUE)
...
# llm test-----------------------------------------------------
S8_P.llm <- DA.llm(S8_phylum, predictor = "SampleDay", paired = "Fecal.ID", allResults = TRUE)
...
# Identify Diferential phylum between Day 0 and all other days ---------------
# Pull out any rows where Day 0 and Day 1 are significantly different
lmc_P_D0v1 <- lmc.P_tukey_D0vAll[lmc.P_tukey_D0vAll[,6] <= 0.05,]
View(lmc_P_D0v1) # None
llm2_P_D0v1 <- llm2.P_tukey_D0vAll[llm2.P_tukey_D0vAll[,6] <= 0.05,]
View(llm2_P_D0v1) #ASV 4
llm_P_D0v1 <- llm.P_tukey_D0vAll[llm.P_tukey_D0vAll[,6] <= 0.05,]
View(llm_P_D0v1) # None
# Day 0 vs Day 1 ASVs: 4

lmc_P_D0v2 <- lmc.P_tukey_D0vAll[lmc.P_tukey_D0vAll[,7] <= 0.05,]
View(lmc_P_D0v2) # None
llm2_P_D0v2 <- llm2.P_tukey_D0vAll[llm2.P_tukey_D0vAll[,7] <= 0.05,]
View(llm2_P_D0v2) #ASVs: 17, 4
llm_P_D0v2 <- llm.P_tukey_D0vAll[llm.P_tukey_D0vAll[,7] <= 0.05,]
View(llm_P_D0v2) # None
# Day 0 vs Day 2 ASVs: 17, 4

lmc_P_D0v3 <- lmc.P_tukey_D0vAll[lmc.P_tukey_D0vAll[,8] <= 0.05,]
View(lmc_P_D0v3) #ASVs: 4, 2, 1
llm2_P_D0v3 <- llm2.P_tukey_D0vAll[llm2.P_tukey_D0vAll[,8] <= 0.05,]
View(llm2_P_D0v3) #ASVs: 17, 4
llm_P_D0v3 <- llm.P_tukey_D0vAll[llm.P_tukey_D0vAll[,8] <= 0.05,]
View(llm_P_D0v3) #ASVs: 4, 2, 1
# Day 0 vs Day 3 ASVs: 4, 2, 1

lmc_P_D0v4 <- lmc.P_tukey_D0vAll[lmc.P_tukey_D0vAll[,9] <= 0.05,]
View(lmc_P_D0v4) #ASVs: 4, 2
llm2_P_D0v4 <- llm2.P_tukey_D0vAll[llm2.P_tukey_D0vAll[,9] <= 0.05,]
View(llm2_P_D0v4) #ASVs: 4
llm_P_D0v4 <- llm.P_tukey_D0vAll[llm.P_tukey_D0vAll[,9] <= 0.05,]
View(llm_P_D0v4) #ASVs: 4, 2
# Day 0 vs Day 4 ASVs: 4, 2

lmc_P_D0v5 <- lmc.P_tukey_D0vAll[lmc.P_tukey_D0vAll[,10] <= 0.05,]
View(lmc_P_D0v5) #ASVs: 4
llm2_P_D0v5 <- llm2.P_tukey_D0vAll[llm2.P_tukey_D0vAll[,10] <= 0.05,]
View(llm2_P_D0v5) #ASVs: 17, 4
llm_P_D0v5 <- llm.P_tukey_D0vAll[llm.P_tukey_D0vAll[,10] <= 0.05,]
View(llm_P_D0v5) #ASVs: 4

S8_P_tukeyASVs <- c("ASV4", "ASV1", "ASV2", "ASV17")
